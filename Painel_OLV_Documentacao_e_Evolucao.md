...
**Estado atual em 08/08/2026**, conferido contra o banco, o n8n, o Cloudflare e o arquivo do painel nesta data: o painel novo é o sistema único desde a virada de 31/07/2026, acessível em https://painel.olvdistribuidora.com.br. Está em uso real, com **165 pedidos** lançados desde 01/08/2026, **949 clientes**, 18 movimentações de estoque, 44 contas a receber, 29 contas a pagar e 14 baixas. O acesso é individual, com papéis. Contas a Pagar, Contas a Receber, **Contas**, **Resultados** e **Cadastros** estão no ar. **Fluxo de Caixa está construído e funcionando, porém oculto no menu por decisão de Rogério desde 05/08/2026** (seção 5.8).

**O módulo de Fornecedores entrou em 08/08/2026** e é a novidade desta versão: cadastro próprio, ligação por identificador em contas a pagar e em movimentações de estoque, e filtro por fornecedor no histórico do estoque. Detalhes nas seções 3.15, 4.3 e 5.4.

**O Worker `painel-olv` no Cloudflare foi atualizado pela última vez em 08/08/2026, às 14h36 UTC**, depois das migrações do dia, que terminaram às 14h16. O arquivo no ar é o que contém o módulo de Fornecedores. O número da versão do HTML **não existe dentro do arquivo**: procurado linha a linha na conferência desta versão e não encontrado. Ver a proposta de marcador na seção 11.
...
**Status em 08/08/2026**: conexão n8n para Supabase ativa e testada. Esquema do banco e **sete endpoints OLV2** em produção, quatro deles republicados em 08/08/2026 para o módulo de Fornecedores. Os três workflows antigos de dados estão desativados e guardados como pontos de retorno. OLV Login e OLV Contas continuam ativos porque são a camada de acesso compartilhada. O navegador não fala direto com o Supabase.
...
Cada assunto vive em uma tabela própria, ligada às outras por um código (ID). **Dez tabelas de movimento, sete de cadastro e cinco views de apoio**, ou seja **17 tabelas ao todo**, conferidos no banco em 08/08/2026. Tranca de segurança (RLS) ativa em todas as 17, nenhuma com política, que é o desenho pretendido e está explicado na seção 3.10.

As dez de movimento guardam o que acontece: clientes, produtos, vendas, venda_itens, pagamentos, estoque_movimentacoes, contas_pagar, contas_receber, baixas e **contas_ajustes**, esta última criada em 03/08/2026 e detalhada na seção 3.14. As sete de cadastro guardam as regras e os parâmetros do negócio: seis criadas em 31/07/2026, na seção 3.12, mais **`fornecedores`**, criada em 08/08/2026 e detalhada na seção 3.15.
...
**Gás do Povo mudou de lado em 05/08/2026 (migração `gas_do_povo_como_cartao`).** Até 04/08 ele gerava recebível de verdade, com vencimento em 3 dias úteis e conta escolhida na baixa. Passou a ser tratado como recebimento de terceiro, igual ao cartão: `gera_recebivel` desligado, `e_de_terceiro` ligado, `prazo_credito_dias` em 3, `prazo_em_dias_uteis` ligado e `conta_destino_id` apontando para o Caixa da Loja. O Fluxo de Caixa projeta direto de `pagamentos`, e ninguém dá baixa manual em repasse que cai sozinho.

**O que essa mudança custa**: se o repasse do programa atrasar ou vier diferente do previsto, o sistema **não tem mais onde registrar essa diferença**, porque não existe recebível para baixar parcialmente. Era exatamente esse o argumento que, em 02/08, justificou tratar Gás do Povo como recebível. **Ponto em aberto para Rogério**, registrado como pendência 26 na seção 3.11.

A correção de 02/08/2026 havia reparado as vendas de Gás do Povo que tinham ficado sem recebível, sem duplicar as que já estavam corretas. Os recebíveis criados naquela janela continuam no banco e seguem o ciclo normal de baixa.
...
| data | Data | Começa com hoje |
| fornecedor | Texto | Opcional. **Nome copiado no momento do lançamento.** Continua sendo gravado ao lado do `fornecedor_id`, ver pendência 29 |
| fornecedor_id | Ligação | **Criado em 08/08/2026.** Aponta para `fornecedores`. É o campo que o filtro do histórico usa. Seção 3.15 |
...
|-------|------|-------|
| id | ID | Chave |
| fornecedor, descricao, categoria | Texto | Opcionais. `fornecedor` é o **nome copiado** no momento do lançamento, ver pendência 29 |
| fornecedor_id | Ligação | **Criado em 08/08/2026.** Aponta para `fornecedores`, **sem cascata**: fornecedor com conta ligada não pode ser apagado. Seção 3.15 |
...
**Fornecedor virou cadastro em 08/08/2026** (migrações `criar_pagar_com_fornecedor_id` e `editar_pagar_com_fornecedor_id`). As duas funções passaram a receber `fornecedor_id` junto com o nome, e a `vw_contas_pagar` passou a devolver a coluna. O campo da tela virou uma caixa com lista de sugestões: escolher um fornecedor já cadastrado preenche o identificador, e digitar um nome novo cadastra o fornecedor sozinho antes de salvar a conta. Ver 3.15 e 5.4.
...
| status | Texto | **Validado**: Em aberto, Parcial ou Recebido. **Nunca digitado desde 01/08/2026**: é calculado pela soma das baixas contra o valor |
...
**Lançamento manual passou a existir em 01/08/2026.** `cliente_id`, `venda_id` e `pagamento_id` aceitam vazio, então dívida que não nasceu de pedido do painel (acerto antigo, venda feita fora do sistema, empréstimo) entra pela tela. Quem tem `venda_id` ou `pagamento_id` preenchido veio de pedido e **não pode ser excluído pelo financeiro**: desfaz-se pelo pedido de origem, senão a venda e o financeiro passam a contar histórias diferentes.

### 3.13 baixas (criada em 01/08/2026)

Recebimentos e pagamentos parciais. É a pendência 18, que era a decisão 1 da seção 9.4.

| Campo | Tipo | Regra |
|-------|------|-------|
| id | ID | Chave |
| conta_receber_id | Ligação | Aponta para contas_receber, **com cascata** |
| conta_pagar_id | Ligação | Aponta para contas_pagar, **com cascata** |
...
**Quitação de dívida antiga no cartão, com taxa (22/08/2026).** Até esta data, quando um cliente pagava um crediário antigo no débito ou crédito, o valor da baixa entrava cheio, sem descontar a taxa da maquininha, diferente do que já acontecia numa venda nova no cartão. A tabela `baixas` ganhou as mesmas colunas de cartão que `pagamentos` já tinha (maquininha, taxa aplicada, grupo, modalidade, parcelas, valor da taxa e valor líquido, calculadas sozinhas pelo banco), com uma trava nova garantindo que essas colunas só existem em baixa de Contas a Receber, nunca de Contas a Pagar. `registrar_baixa` e a nova `registrar_baixas` (várias formas numa baixa só) capturam a taxa do mesmo jeito que a venda já fazia: resolvem a maquininha sozinhas quando só existe uma ativa, e nunca bloqueiam a baixa por falta de taxa cadastrada, só avisam. A `conta_movimentos` passou a creditar o valor líquido na conta, não o cheio, no cartão. **Correção vale só daqui para frente**: baixa lançada antes de 22/08/2026 continua exatamente como estava, sem taxa. **A dívida em si continua sendo considerada quitada pelo valor cheio**; só o que entra na conta do banco é o líquido. Decisão do Rogério, escopo fechado só em Contas a Receber: Contas a Pagar não muda.

### 3.14 contas_ajustes (criada em 03/08/2026)

Movimentos de conta que **não nasceram de venda, de baixa ou de conta a pagar**: tarifa bancária, rendimento, transferência que o sistema não conhece, e a diferença encontrada ao conferir o saldo com o extrato.

| Campo | Tipo | Regra |
|---|---|---|
| id | ID | Chave |
| conta_id | Ligação | Aponta para contas, com RESTRICT |
| data | Data | Padrão é hoje |
| natureza | Texto | **Validado**: Crédito ou Débito |
| valor | Número | **Maior que zero**, validado |
| origem | Texto | **Validado**: Ajuste manual ou Conciliação. Padrão é Ajuste manual |
| descricao | Texto | Obrigatória, não pode ser só espaço |
| saldo_informado | Número | Só existe na Conciliação. É o saldo que Rogério leu no banco ou contou no caixa |
| responsavel | Texto | Quem lançou. Vem do token, nunca do navegador |
| criado_em | Data e hora | Preenchido sozinho |

**A trava que amarra as duas origens**: `contas_ajustes_conciliacao_check` exige `saldo_informado` preenchido quando a origem é Conciliação, e exige vazio quando é Ajuste manual. Sem ela, um ajuste manual poderia se disfarçar de conciliação e o histórico deixaria de dizer de onde veio o número.

**Conciliar não é digitar o saldo.** A função `conta_conciliar` calcula o saldo do sistema até a data informada, compara com o saldo que Rogério informou e **grava só a diferença**, como uma linha de ajuste. É a mesma escolha da pendência 13 e da tabela de baixas: valor derivado de histórico editável não se guarda de cabeça, se recalcula. Sobrescrever o saldo apagaria a pergunta que interessa, que é onde a diferença apareceu.

**Duas recusas por desenho**: a função recusa conta sem `data_inicial` preenchida, porque sem ponto de partida não existe saldo calculável; e recusa data anterior à `data_inicial`, porque conferir antes do início não significa nada.

**Funções da seção**: `contas_consultar` (saldos e movimentos), `saldo_conta_calculado` (saldo de uma conta numa data), `conta_ajustar`, `conta_conciliar` e `estornar_ajuste_conta`. Todas exclusivas do administrador, roteadas pelo `financeiro_operar`.

**Estado em 07/08/2026**: a tabela existe e está vazia, zero lançamentos. Três das quatro contas já têm `saldo_inicial` e `data_inicial` preenchidos, o que é a pendência 21 quase fechada. Ver seção 3.11.

### 3.15 fornecedores (criada em 08/08/2026)

Até aqui, fornecedor era só um texto digitado a cada lançamento. Cada Entrada de estoque e cada conta a pagar guardava o nome escrito na hora, então "Ultragaz", "ultragaz" e "Ultra Gaz" eram três fornecedores diferentes para o sistema, e não havia como somar o que foi comprado de cada um.

A tabela resolve isso. É o mesmo movimento que já tinha sido feito com clientes e produtos: o que se repete vira cadastro, e o lançamento passa a apontar para ele.

| Campo | Tipo | Regra |
|-------|------|-------|
| id | ID | Chave |
| nome | Texto | **Obrigatório e único no banco.** Não existem dois fornecedores com o mesmo nome |
| telefone | Texto | Opcional |
| observacao | Texto | Opcional |
| ativo | Sim/Não | Começa em Sim |
| criado_em | Data e hora | Preenchida pelo banco |

**Seis fornecedores cadastrados em 08/08/2026.**

**Quem aponta para cá**: `contas_pagar.fornecedor_id` e `estoque_movimentacoes.fornecedor_id`, as duas ligações **sem cascata**. Apagar um fornecedor que já tem lançamento é recusado pelo banco, pelo mesmo motivo que protege cliente e produto: o histórico não pode sumir por um clique no cadastro.

**Cadastro na hora, sem sair da tela.** Nas duas telas que pedem fornecedor, o campo mostra a lista dos já cadastrados e aceita um nome novo. Quando o nome digitado não existe, o painel cria o fornecedor antes de gravar o lançamento, e só então grava. É o que evita a tela pedir que a pessoa pare o que está fazendo, vá até Cadastros, crie o fornecedor e volte.

**Auditoria de permissão desta criação automática, conforme a regra 8.** A criação chama o endpoint `olv2-cadastros`, que é **exclusivo do administrador**. Se um colaborador conseguisse chegar até ali, o lançamento falharia com "Cadastros são exclusivos do administrador". **Não chega**: o campo Fornecedor só aparece quando o lançamento exige custo, ou seja Entrada e Estoque Inicial, e esses dois tipos já são exclusivos do administrador desde 30/07/2026 (seção 7). O colaborador só lança Ajuste, que não mostra o campo. Na tela de Contas a Pagar, a própria tela é exclusiva do administrador. Conferido no arquivo do painel em 08/08/2026, caminho por caminho.
...
fornecedores ──→ estoque_movimentacoes
fornecedores ──→ contas_pagar

contas ──→ contas_ajustes
contas ──→ baixas (conta de destino)
```
...
**`vw_resultados` (criada em 05/08/2026, ajustada em 06/08/2026 e em 22/08/2026)**: a quinta view, que alimenta a tela Resultados. Nasceu **aditiva**: sem alterar nenhuma view, função ou workflow existente, o que era a condição para construí-la sem risco para a operação.

Ela junta origens numa lista só, com uma linha por evento:

| Tipo de linha | De onde vem | O que conta como valor |
|---|---|---|
| `Venda` | `vendas`, com itens e pagamentos agregados | Só a parte **à vista** do pedido. **Desde 22/08/2026, também exclui a fatia paga em Vale-Crédito**, porque ela já foi contada como faturamento no dia em que o crédito foi gerado, não no dia em que é gasto |
| `Recebimento` | `baixas` ligadas a `contas_receber` | O valor da baixa, na data em que o dinheiro entrou. **Desde 22/08/2026, desconta a taxa do cartão do lucro**, igual à linha de Venda |
| `Geração Vale-Crédito` (22/08/2026) | `creditos_movimentacoes`, tipo `ENTRADA` | O valor gerado, no dia em que o dinheiro entra. Se a geração foi no cartão, a taxa já sai descontada do lucro aqui, encerrando a pendência 32: antes a taxa da geração no cartão não aparecia em lugar nenhum |
| `Estorno Vale-Crédito` (22/08/2026) | `creditos_movimentacoes`, tipo `ESTORNO_ENTRADA` | O valor estornado, como lançamento negativo. Um pedido pago 100% em Vale-Crédito aparece com **lucro negativo** no dia em que o crédito é gasto, igual ao custo do produto: é esperado, porque o lucro dele já foi contado na geração, não de novo no consumo |

**Por que separar à vista de a prazo.** Sem isso, um pedido de R$ 220 pago metade em dinheiro e metade em crediário apareceria inteiro no dia da venda e **de novo** no dia do recebimento. Contando só a parte à vista na linha da venda, e a parte a prazo na linha do recebimento, cada real aparece uma vez só. A coluna `a_receber` mostra quanto daquele pedido ficou para depois.

**O custo só entra quando o estoque baixou.** Enquanto o pedido não estiver Concluído com `data_conclusao`, o custo da linha é zero, pelo mesmo critério da seção 6.
...
| 20 | ~~Conta de destino não preenchida~~ | **Resolvida, conferida no banco em 07/08/2026**: Dinheiro e Gás do Povo apontam para o Caixa da Loja; Pix, Débito e Crédito apontam para o Banco Itaú. Crediário, Em aberto e Gratuidade seguem vazios, que é o correto, porque a conta é escolhida na baixa ou não existe dinheiro |
| 21 | **Saldo inicial do Caixa da Loja, e o susto que ele vai causar** | Reconferido no banco em 08/08/2026, sem mudança: Itaú (R$ 1.061,21), Sicredi (R$ 1.879,15) e Banco Digital (R$ 0,00) têm `data_inicial` em 02/08/2026. **O Caixa da Loja segue com saldo 0,00 e `data_inicial` vazia.** Enquanto isso, a conciliação da seção 3.14 **recusa** essa conta, porque sem data inicial não há de onde partir a conta do saldo. É a única conta física e a que mais recebe dinheiro na hora. **Reforçado em 08/08/2026, ao encerrar a pendência 28**: o financeiro ainda não fez nenhum lançamento de ajuste, então a primeira tentativa dele muito provavelmente vai ser justamente no Caixa da Loja, e vai ser recusada. A leitura natural de quem receber essa recusa é "a tela não funciona", quando o que falta é um cadastro de dois campos que só depende de Rogério. **É o item de maior risco de virar desconfiança na ferramenta, e o mais barato de resolver** |
...
| 26 | **Gás do Povo virou cartão e perdeu o lugar do atraso** | **Aberta em 07/08/2026, na conferência desta versão.** Desde 05/08 o Gás do Povo não gera mais recebível, é projetado como cartão em 3 dias úteis. Se o repasse do programa atrasar ou vier diferente, não existe recebível para baixar parcialmente e a diferença não tem onde ser registrada. Era esse o argumento que justificou o desenho anterior, em 02/08. **Decisão de Rogério.** Ver seção 3.5 |
...
| 32 | ~~Taxa do cartão some do lucro do pedido pago com crédito~~ | **Resolvida em 22/08/2026.** A geração do Vale-Crédito passou a contar em Resultados no dia em que o dinheiro entra, já com a taxa do cartão descontada quando for o caso. A taxa não some mais: ela é contada uma vez, na geração. Ver seção 9.4 e a decisão 15 da tabela do Vale-Crédito, superada nesta data |
...
**A sétima tabela de cadastro, `fornecedores`, entrou em 08/08/2026** e tem seção própria, a 3.15, porque não nasceu com as outras seis e a regra de exclusão dela é diferente.

**Colunas acrescentadas a `formas_pagamento` depois da criação**, conferidas no banco em 08/08/2026:

| Coluna | Para que serve |
|---|---|
| `e_de_terceiro` | Marca a forma em que quem paga é uma operadora ou um programa, não o cliente. Ligada em Débito, Crédito e Gás do Povo |
| `prazo_credito_dias` | Quantos dias até o dinheiro cair. 0 em Dinheiro e Pix, 1 no cartão, 3 no Gás do Povo |
| `prazo_em_dias_uteis` | Diz se o prazo acima conta dias úteis ou dias corridos |
| `conta_destino_id` | Conta onde o dinheiro entra quando a forma recebe na hora |
| `taxa_padrao_pct` | Taxa da própria forma, quando não vem de maquininha |

**Migração de 05/08/2026, `liberar_campos_comportamento_formas_pagamento`**: mais campos de comportamento passaram a ser editáveis pela tela de Cadastros. O risco aceito é o mesmo já registrado nesta seção, e ficou maior: quanto mais comportamento vira campo, mais combinações plausíveis e erradas cabem na tela, e as travas só cobrem as contraditórias.
...
**Auditoria da instância em 08/08/2026**: **20 workflows**, sendo 7 de dados OLV2 ativos, 2 de acesso ativos, 3 antigos desativados como ponto de retorno, 3 cópias de backup desativadas e 5 de outros assuntos. Nenhum workflow foi criado nem excluído desde a v5.1. A instância continua com uma única credencial de Supabase, do tipo Postgres.

**Publicação de 08/08/2026, módulo de Fornecedores.** Quatro workflows republicados na mesma janela:

| Workflow | Horário (UTC) | O que mudou |
|---|---|---|
| OLV2 Cadastros | 14h05 | Passou a aceitar a tabela `fornecedores`. A lista de tabelas válidas do nó `Prep Cadastro` foi de sete para oito |
| OLV2 Estoque | 14h06 | A Entrada passou a repassar `fornecedor_id` junto com o nome |
| OLV2 Financeiro | 14h07 | `criar_pagar` e `editar_pagar` passaram a repassar `fornecedor_id`; `listar_pagar` passou a devolver a lista de fornecedores para preencher a caixa de sugestões |
| OLV2 Clientes | 14h12 | Republicado na mesma janela |

**Desta vez os identificadores não mudaram**, e isso corrige a leitura que a v5.1 poderia induzir. A regra da seção 4.2 não é "publicar sempre troca o identificador": trocar acontece quando a publicação **substitui o workflow ativo por uma cópia nova**, que foi o caso do Estoque e do Financeiro em 02/08. Quando a publicação atualiza o workflow no lugar, como em 08/08, o identificador continua o mesmo. **A regra que fica é a mesma de antes, e por isso continua valendo**: confira o nome antes de auditar por identificador, porque você não sabe de que tipo foi a última publicação.

**Divergência de rótulo registrada em 08/08/2026**: a descrição do próprio workflow OLV2 Cadastros, dentro do n8n, ainda lista sete tabelas e não cita fornecedores. O código já aceita oito. É rótulo desatualizado, não comportamento errado, e vale corrigir na próxima passada para não enganar quem for auditar pela descrição.

**Identificadores mudaram e o documento estava desatualizado.** Publicar substituindo o workflow ativo por uma cópia nova cria um identificador novo e aposenta o antigo. Foi o que aconteceu com Estoque e Financeiro:

| Workflow | ID no documento até a v5.0 | ID real em 07/08/2026 | Cópia guardada |
|---|---|---|---|
| OLV2 Estoque | `3l15lOfGCeYqLEu7` | `V3LHZ9kYui2NMMu0` | BACKUP OLV2 Estoque `gwKsvNj6AbBvho9B` |
| OLV2 Financeiro | `q4Bbzx7BSn6EiwjQ` | `ASx4FI8ZZEFcmv07` | BACKUP OLV2 Financeiro `WqwpnFh4yHu7LPIG` e BACKUP OLV2 Financeiro v10 `nGZFU6PchJgh7rmW` |

**Regra que fica**: publicação por substituição troca o identificador. Quem for conferir um workflow pelo ID escrito neste documento precisa confirmar o nome antes, senão vai auditar uma cópia desativada achando que é a versão em produção.
...
Workflow ID `fV9fzAM81cT2Pvma`. Cobre, desde 08/08/2026, **oito** tabelas de cadastro: `produtos` (edição do `preco_venda`), `contas`, `maquininhas`, `bandeiras`, `taxas_cartao`, `formas_pagamento`, `categorias_conta_pagar` e **`fornecedores`** (seções 3.12 e 3.15).
...
### 4.4 OLV2 Financeiro (construído em 01/08/2026)

Identificador **`ASx4FI8ZZEFcmv07`**, endereço `POST /webhook/olv2-financeiro`. (Até a v5.1 esta seção repetia o identificador antigo `q4Bbzx7BSn6EiwjQ`, que pertence a uma cópia desativada. Corrigido em 08/08/2026; a seção 4.2 já trazia o número certo.) Mesmo bloco de entrada dos outros cinco endpoints, nó a nó: validação do token por SHA256, `allowedOrigins` restrito à origem do painel, e o erro devolvido como resposta em vez de falha de nó.

| Ação | O que faz |
|---|---|
| `listar` | Devolve os recebimentos com o histórico de baixas de cada um, mais o resumo, as contas ativas e as formas aceitas. Filtra por situação, por vencimento e por texto |
| `criar` | Lança recebimento manual, sem pedido de origem |
| `excluir` | Apaga recebimento manual que ainda não teve nenhuma baixa |
| `baixar` | Chama a `registrar_baixa` (uma forma) ou `registrar_baixas` (várias formas numa baixa só, 22/08/2026), com captura da taxa do cartão quando a forma é Débito ou Crédito |
| `estornar` | Chama a `estornar_baixa` |
| `listar_pagar` | Devolve contas a pagar, baixas, resumo do período, contas ativas, categorias, formas aceitas e, **desde 08/08/2026, a lista de fornecedores** |
...
| `contas` | **03/08/2026.** Chama `contas_consultar`: saldos e movimentos de cada conta |
| `ajustar_conta` | **03/08/2026.** Chama `conta_ajustar`: lança crédito ou débito avulso numa conta |
| `conciliar_conta` | **03/08/2026.** Chama `conta_conciliar`: compara o saldo informado com o calculado e grava só a diferença |
| `estornar_ajuste_conta` | **03/08/2026.** Apaga um ajuste, e o saldo volta sozinho pela releitura |
...
**Fluxo de Caixa acrescentado em 02/08/2026.** A ação `fluxo_caixa` usa o mesmo endpoint e chama `fluxo_caixa_consultar`. O workflow continua exclusivo do administrador e mantém as duas travas em série. A publicação foi feita substituindo o workflow ativo por uma nova cópia, e a versão anterior ficou desativada como backup. **Efeito colateral registrado**: o identificador mudou, ver o quadro de IDs na seção 4.2.

**Seção Contas acrescentada em 03/08/2026** (migração `financeiro_rotear_secao_contas`). Quatro ações novas, todas roteadas pelo mesmo `financeiro_operar`, mantendo as duas travas de administrador em série. Detalhe da tabela e das regras na seção 3.14.

**Filtro por campo de data e busca por pedido, em 07/08/2026** (migração `financeiro_operar_campo_data_e_busca_pedido`). Duas mudanças no `listar` e no `listar_pagar`:

| Mudança | O que faz |
|---|---|
| `campo_data` | Escolhe se o período filtra por **vencimento** (padrão), **emissão** ou **pagamento**. Valor desconhecido cai em vencimento, que é o comportamento anterior |
| Busca por número do pedido | O termo aceita o número, com ou sem "#". Casa por igualdade, não por "contém", para "12" não trazer 12, 120 e 128 |

**Detalhe do filtro por pagamento**: quando `campo_data` é pagamento, a lista passa a ter **uma linha por baixa**, não por conta. A mesma conta aparece duas vezes se tiver baixas em períodos diferentes, e por isso a chave que viaja é `baixa_id`, não o id da conta. Quem consumir essa resposta precisa saber disso, senão vai deduplicar por engano e perder recebimento.

---

### 4.5 OLV2 Resultados (construído em 05/08/2026)

Identificador `d2rKeJd5vbzNOJpj`, endereço `POST /webhook/olv2-resultados`, ativo. Uma ação só, `resultados`. Exclusivo do administrador, com a trava positiva sobre `admin` conforme a regra da seção 8.2.

**Nasceu aditivo, de propósito**: não alterou nenhum workflow, view ou função existente. Lê apenas a `vw_resultados`, criada junto. Com o sistema em operação real desde 01/08, tela nova que mexe em caminho de produção é risco desnecessário quando dá para construir ao lado.

Mesmo bloco de entrada dos outros seis endpoints, nó a nó: Normalizar, Autenticar, Assinar (Crypto com `type: SHA256` e `encoding: hex` gravados), Autorizar, Autorizado?, consulta, montagem e resposta, com `allowedOrigins` restrito à origem do painel e cabeçalho `Cache-Control: no-store`.

**Os filtros de cliente, forma e produto rodam em JavaScript, não no SQL.** É a aplicação direta da armadilha da seção 8.5: o `queryReplacement` do nó Postgres separa por vírgula, então filtro vazio some da lista de parâmetros e derruba a consulta com "there is no parameter $3", e nome de cliente com vírgula desloca os parâmetros seguintes. Só o período vai como parâmetro. Filtrar em JavaScript é comparação exata sobre dados já lidos, então não abre porta para injeção.

**A busca por cliente também aceita o número do pedido (07/08/2026)**, com ou sem "#". O número casa por igualdade, não por "contém", para "12" não trazer 12, 120 e 128 juntos. É a mesma regra da tela Pedidos.

**Duas colunas são removidas antes de responder**: `formas_filtro` e `produto_ids` servem só para o filtro do lado do servidor e não viajam para o navegador.
...
### 5.0 O menu de verdade (conferido em 08/08/2026)

Este é o menu como ele existe hoje, lido na constante `ITENS_MENU` do arquivo do painel. Vale como índice das seções abaixo.
...
**Não confie no corpo do HTML para responder esta pergunta.** O arquivo carrega um retrato antigo da tela já desenhada, e esse retrato mostra um menu de 31/07/2026 com metade dos itens marcados como "Em construção". Ele é substituído assim que a página abre. Motivo e detalhe na seção 8.7.
...
**Campo Fornecedor com lista de sugestões, desde 08/08/2026.** Aparece na Entrada e no Estoque Inicial, que são os lançamentos que pedem custo e, por isso, os exclusivos do administrador. Escolher um fornecedor já cadastrado preenche a ligação; digitar um nome novo cadastra o fornecedor sozinho ao salvar. Ver 3.15.

**Filtro por fornecedor em Movimentações, desde 08/08/2026.** O filtro é montado a partir dos fornecedores que aparecem nas movimentações já carregadas do período e recorta a lista no próprio navegador, sem nova consulta ao servidor. Só entram no filtro as movimentações que têm `fornecedor_id`: as anteriores a 08/08/2026 guardaram apenas o nome em texto e não aparecem nele.
...
Tela exclusiva do administrador. Desde 08/08/2026 cobre **oito** abas: produtos (edição do preço de venda sugerido), contas, maquininhas, bandeiras, taxas de cartão, formas de pagamento, categorias de conta a pagar e **fornecedores**. Maquininhas mostram e editam os prazos do débito e do crédito em dias úteis.

**Aba Fornecedores, acrescentada em 08/08/2026.** Nome (obrigatório e único), telefone, observação e ativo. É a oitava e última aba. A criação também acontece por fora desta tela, direto no lançamento de Contas a Pagar e na Entrada de estoque, quando a pessoa digita um nome que ainda não existe. Ver 3.15.
...
### 5.6 Contas a Receber (construída em 01/08/2026)
...
**Quitação no cartão, com bandeira e parcelas (22/08/2026).** Quando a forma escolhida em Receber é Débito ou Crédito (cliente quitando um crediário antigo no cartão, por exemplo), aparece o mesmo interruptor "Bandeira Outras (Elo)" do formulário de pedido, e no Crédito também as parcelas (1x, 2x, 3x). A taxa da maquininha passa a ser descontada do valor que entra na conta, do mesmo jeito que já acontecia numa venda nova no cartão. A dívida continua sendo considerada quitada pelo valor cheio. Ver seção 3.13.

**Ver resumo do pedido, clicando na linha (achado e corrigido em 22/08/2026).** Clicar em cima de uma linha com pedido de origem abre o mesmo resumo da tela Pedidos. Até esta data, abrir esse resumo direto por aqui, sem antes ter visitado a tela Pedidos, Vendas ou Resultados nesta sessão, dava **"Pedido não encontrado" em todas as linhas**, porque o resumo procurava o pedido só nos dados dessas outras três telas. Corrigido: agora o resumo também usa os dados que já estão na própria lista de Contas a Receber. Detalhe na seção 8.9.

**Incluir** lança dívida que não nasceu de pedido. **Excluir** só aparece em lançamento manual que ainda não teve recebimento.

**O que não existe nesta tela, de propósito:**
...
### 5.8 Fluxo de Caixa (construído em 02/08/2026, oculto no menu desde 05/08/2026)

**Oculto por decisão de Rogério em 05/08/2026, confirmada em 08/08/2026.** A tela **não foi removida**: a função que a desenha, a que carrega os dados, a rota de navegação e o filtro do topo continuam inteiros no arquivo. O que existe é uma marca `oculto: true` na linha do item dentro do `ITENS_MENU`, e o próprio código traz o comentário dizendo que reativar é apagar essa marca, nada mais. Enquanto isso, o Fluxo de Caixa não aparece no menu para ninguém, nem para o administrador.

Até a v5.1 este documento afirmava que a tela estava no ar, o que estava errado desde 05/08. Corrigido aqui.

O restante desta seção descreve a tela como ela funciona quando reativada.
...
### 5.10 Contas (construída em 03/08/2026)

Tela exclusiva do administrador, ligada ao OLV2 Financeiro. Mostra o saldo de cada conta e os movimentos que o formaram, e permite duas ações que o resto do sistema não cobre.

**Ajustar** lança um crédito ou um débito avulso, com descrição obrigatória. Serve para tarifa bancária, rendimento e transferência que o painel não conhece.

**Conciliar** é a conferência com o extrato. Rogério informa o saldo que está no banco ou contado no caixa, e o sistema calcula o saldo dele até aquela data e **grava só a diferença**. O saldo nunca é sobrescrito, pelo motivo da seção 3.14.

**Estornar** apaga um ajuste, e o saldo volta sozinho.

**Duas recusas por desenho**: conta sem saldo e data inicial preenchidos não pode ser conciliada, e data anterior à data inicial é recusada. Hoje isso deixa o **Caixa da Loja de fora**, que é a pendência 21.

**Estado em 08/08/2026**: a tela está no ar, é encontrada e funciona. A tabela `contas_ajustes` continua com **zero lançamentos** porque **o financeiro ainda não fez os ajustes**, confirmado por Rogério nesta data. Não há nada a corrigir na tela.

**O que provavelmente acontece no primeiro uso.** O Caixa da Loja está sem saldo e data iniciais, e as duas recusas por desenho, acima, valem para ele. Quem tentar conciliar essa conta primeiro vai receber uma recusa e pode concluir que a tela está quebrada. Preencher os dois campos no cadastro de contas resolve. É a pendência 21.

### 5.11 Resultados (construída em 05/08/2026)

Tela exclusiva do administrador, ligada ao endpoint OLV2 Resultados. É a visão de resultado por evento: cada linha é uma venda ou um recebimento, na data em que o dinheiro se moveu.

Filtros de período, cliente, forma de pagamento e produto. O campo de cliente **também aceita o número do pedido**, com ou sem "#" (07/08/2026).

**A soma da tela não é a soma do Dashboard, e isso é proposital.** O Dashboard conta faturamento por pedido concluído. Resultados conta dinheiro por evento: a parte à vista entra no dia da venda, a parte a prazo entra no dia do recebimento. Quem comparar os dois números lado a lado vai encontrar diferença, e a diferença é exatamente o que ainda está a receber.
...
**Duas linhas novas de tipo, desde 22/08/2026: Geração Vale-Crédito e Estorno Vale-Crédito.** O filtro de Tipo ganhou as duas opções, com selo de cor própria na tabela. A geração do crédito passou a contar como faturamento e lucro no dia em que o dinheiro entra, não só no dia em que o cliente gasta; o consumo depois conta só o custo do produto. Ver seção 3.9 e a decisão 15 da tabela do Vale-Crédito (seção 10), superada nesta data.
...
`cadastro_operar` era a única função com `search_path` livre, único aviso de nível WARN do auditor do Supabase. Fixado em `public, pg_temp` sem tocar no corpo, porque a função usa nomes sem schema. Restam avisos informativos do tipo `rls_enabled_no_policy`, que são o desenho e não falha. Eram 8 na v3.5, passaram a 14 quando as 6 tabelas de cadastro entraram, 16 em 07/08/2026 e estão em **17 em 08/08/2026**, com a `fornecedores` somada. **Nenhum aviso de nível WARN ou superior**, conferido nesta data. A auditoria `select * from auditar_acesso_publico() where anon or autenticado or publico;` devolveu **zero linhas** depois das seis migrações do dia, conforme a regra 9.

---

### 8.7 O arquivo do painel mente sobre si mesmo para quem não roda JavaScript (achado em 08/08/2026)

**Como apareceu.** Na conferência desta versão, a leitura do endereço do painel devolveu um menu com oito itens, três deles marcados como "Em construção", e sem Pedidos, Cadastros, Contas nem Resultados. Isso contradizia o documento inteiro. A conclusão fácil e errada seria: o arquivo publicado está atrasado.

**O que era.** O arquivo do painel guarda, dentro do `<body>`, **um retrato de uma tela já desenhada**, congelado em 31/07/2026. Dá para datar sem margem de dúvida: o campo de data do Dashboard nesse retrato vem com o valor `2026-07-31` escrito. O menu de verdade não está nesse retrato. Ele é montado pelo script, a partir da constante `ITENS_MENU`, e substitui o retrato assim que a página abre.

**Por que isso importa.** Qualquer leitura que não execute JavaScript enxerga o painel como ele era em 31/07: ferramenta de leitura de página, buscador, verificação automática, e a própria IA numa sessão de auditoria. É a mesma família de armadilha das seções 8.2 e 8.5: a informação está lá, disponível, e leva para a conclusão errada.

**Regra que fica**, e que virou a regra 13 das instruções: para saber quais telas o painel tem, leia a constante `ITENS_MENU`. O corpo do HTML não responde essa pergunta.

**Recomendação, não aplicada**: o retrato antigo não serve para nada em produção, já que é descartado no primeiro desenho da tela. Removê-lo deixaria o arquivo menor e acabaria com a armadilha de vez. Fica registrado como item a fazer, não como pendência numerada, porque não quebra nada hoje.
...
### 8.9 "Pedido não encontrado" em Contas a Receber, e o resumo que só olhava para três telas (achado e corrigido em 22/08/2026)

**Como apareceu.** Rogério reportou que, ao clicar em cima de qualquer linha em Contas a Receber para ver o resumo do pedido, a tela mostrava "Pedido não encontrado" em todos os títulos, sem exceção. Aconteceu logo depois da publicação da manhã (quitação de dívida no cartão, seção 3.13), o que apontava primeiro para essa mudança como causa.

**O que não era.** Conferido no n8n: todas as chamadas do dia ao endpoint financeiro tinham terminado com sucesso, sem erro nenhum. O banco e o servidor estavam corretos. O problema não estava em nenhum dado, estava só na tela.

**O que era de fato.** O resumo do pedido (o mesmo popup usado nas telas Vendas, Pedidos e Resultados) procura o pedido em três lugares: a carga do dia, o filtro de Concluídos e a Lista de pedidos, mais um quarto lugar criado em 07/08/2026 para a tela Resultados. **Contas a Receber nunca tinha entrado nessa lista de lugares.** Quem abrisse o painel e fosse direto para Contas a Receber, sem antes visitar Pedidos, Vendas ou Resultados nesta mesma sessão, encontrava o pedido em lugar nenhum, e a tela mostrava a mensagem de erro em toda linha, porque nenhuma delas achava seu pedido. Isso já acontecia antes de 22/08/2026; a publicação da manhã só tornou mais provável que alguém entrasse direto em Contas a Receber para testar a novidade, o que expôs o problema pela primeira vez.

**Corrigido**, no mesmo padrão já usado para a tela Resultados em 07/08/2026: o resumo passou a também procurar o pedido nos próprios dados que a tela Contas a Receber já tinha em mãos (cliente, descrição, valor e data da dívida). Itens e forma de pagamento do pedido continuam não vindo por esse caminho; o resumo já sabia mostrar "os itens deste pedido não vieram na busca do período" para esse caso, então a tela não finge ter uma informação que não tem.

**Cuidado extra aplicado.** Como administrador, o botão "Editar pedido" apareceria mesmo no resumo aberto por este caminho novo, e editar sem os itens reais gravaria o pedido com um único item em branco por cima do que já existia. Editar por este caminho poderia apagar itens de verdade sem querer. O botão "Editar pedido" agora só aparece quando os itens do pedido realmente vieram junto; quando não vieram, a tela avisa para abrir pela tela Pedidos ou Vendas. Excluir continua disponível, porque a exclusão usa só o número do pedido e não depende dos itens estarem carregados.

**Também corrigido**: a publicação da manhã de 22/08/2026 tinha esquecido de subir o marcador `CONFIG.VERSAO` (regra 30). O número ficou parado em `2026-08-14.2` mesmo com o arquivo mudado. Corrigido junto, para `2026-08-22.1`.
...
- 03/08/2026: **seção Contas criada**, com a tabela `contas_ajustes`, as funções de ajuste e conciliação e o roteamento no `financeiro_operar`. Seções 3.14, 4.4 e 5.10.
- 04/08/2026: **corrigida a ordem do desconto na `atualizar_venda`** (migração `fix_atualizar_venda_desconto_ordem`) e fechado o acesso público da `contas_ajustes` no padrão da seção 3.10.
- 05/08/2026: **view `vw_resultados` e endpoint OLV2 Resultados criados**, de forma aditiva. Seções 3.9, 4.5 e 5.11.
- 05/08/2026: **Gás do Povo passou a ser tratado como cartão**, deixando de gerar recebível. Seção 3.5 e pendência 26.
- 05/08/2026: mais campos de comportamento de `formas_pagamento` liberados para edição na tela de Cadastros.
- 06/08/2026: `vw_resultados` passou a usar a forma da própria baixa, e não só a do pagamento de origem.
- 07/08/2026: **filtro por campo de data e busca por número do pedido** no financeiro e em Resultados. Seções 4.4 e 4.5.
- 07/08/2026: **painel publicado no Cloudflare**, encerrando a publicação pendente desde 03/08.

#### Falta

- Preencher saldo inicial e data inicial do **Caixa da Loja**, a única conta que falta (pendência 21). É da tela de Cadastros e trava a conciliação dessa conta.
- Decidir o tratamento do atraso no repasse do Gás do Povo (pendência 26).
- Confirmar a versão do arquivo HTML que está no ar, que o servidor não informa.
...
**Status em 07/08/2026**: concluída. O painel é o sistema único desde a virada de 31/07/2026, em https://painel.olvdistribuidora.com.br (Cloudflare Workers), com onze telas no ar: Dashboard, Vendas, Pedidos, Estoque, Movimentações, Clientes, Contas a Pagar, Contas a Receber, Fluxo de Caixa, Contas e Resultados, mais Cadastros e Usuários na administração. O Worker `painel-olv` foi atualizado pela última vez em **07/08/2026**.
...
~~**Status do v13 em 03/08/2026**: ainda falta Rogério publicar o HTML no Cloudflare e testar no endereço de produção.~~ **Superado em 07/08/2026: publicado.** O Worker `painel-olv` foi atualizado nesta data, e o endpoint mais novo do projeto (OLV2 Resultados) foi publicado três minutos antes, o que confirma que o painel no ar já conversa com ele.
...
### 9.8 Melhorias de uso pedidas por Rogério (registradas em 22/08/2026) *

Depois de usar o painel no dia a dia, Rogério enviou quatro prints de tela e uma lista de cinco pedidos de melhoria. **Nada disto foi executado ainda.** Esta seção só registra o pedido, para não se perder entre sessões, com a categorização por dificuldade e o modelo de IA indicado (regra da seção 12).

| # | Pedido | Dificuldade | Modelo indicado |
|---|---|---|---|
| 1 | Tirar do painel a mensagem "Editar pedido: abra pela tela Pedidos ou Vendas" que aparece no resumo de pedido quando ele veio de Contas a Receber (seção 8.9), e deixar todo botão Excluir do painel dentro de uma caixinha vermelha, no mesmo estilo em todas as telas | Simples | Sonnet 5 |
| 2 | Trocar botões de texto por ícones nas telas de Clientes, Contas a Pagar e Contas a Receber, com o desenho seguindo referências visuais que Rogério enviou, redesenhadas no estilo de ícone que o painel já usa | Simples/médio | Sonnet 5 |
| 3 | Filtro de forma de pagamento (por exemplo, só Em aberto ou só Crediário) na tela de Contas a Receber | Médio, porque a forma de pagamento hoje não é um dado separado nessa tela e precisa vir do banco | Sonnet 5 |
| 4 | Fornecedor passar a funcionar como Cliente já funciona: ao digitar, aparece a lista, e depois de escolhido aparecem as opções trocar, editar cadastro e ver histórico. Vale para o formulário de nova movimentação de Estoque e para o de Incluir em Contas a Pagar | Médio/alto, porque mexe em dois formulários e precisa reaproveitar um padrão que já existe para Cliente | Sonnet 5 |
| 5 | Transferência entre contas na tela Contas, do jeito que se transfere dinheiro de uma conta para outra na vida real | Alto: é decisão financeira difícil de desfazer | Opus 5 |

**Por que só o item 5 vai para o Opus.** Os itens 1 a 4 são tela, estilo ou uma consulta sobre uma regra que já existe. O item 5 é o único que decide como o painel vai gravar dinheiro saindo de uma conta e entrando em outra sem deixar sobrar uma perna solta se algo falhar no meio do caminho, que é exatamente o tipo de decisão que a seção 12 reserva para o Opus.

**Ordem combinada com Rogério em 22/08/2026**: os itens 1 a 4 primeiro, numa sessão só de Sonnet 5; o item 5 depois, numa sessão separada de Opus 5, só depois de o Sonnet terminar e a versão dele estar publicada.
...
| 3º | **Fase 1: painel operacional novo** | **Concluída.** No ar como sistema único desde 31/07/2026, em painel.olvdistribuidora.com.br. Upgrade visual publicado, e o Worker atualizado pela última vez em 07/08/2026 |
...
**Fechadas entre 03 e 07/08/2026:**

- ~~Publicar o painel no Cloudflare~~: feito, Worker atualizado em 07/08/2026.
- ~~Pendência 20, conta de destino~~: preenchida nas cinco formas que movimentam dinheiro na hora.
- ~~Seção Contas não existia~~: criada em 03/08/2026, com ajuste e conciliação. Seção 3.14.
- ~~Não havia visão de resultado por evento~~: tela Resultados criada em 05/08/2026, de forma aditiva. Seção 4.5.

**Abertas em 07/08/2026, na conferência desta versão:**

- **Pendência 26**: Gás do Povo virou cartão e o atraso no repasse ficou sem lugar para ser registrado. Decisão de Rogério.
...
| 15 | ~~Resultados conta o Vale-Crédito no dia da venda. A geração não aparece em Resultados.~~ **Superada em 22/08/2026**, com autorização do Rogério: a geração passou a contar em Resultados no dia em que o dinheiro entra; o consumo passou a contar só pelo custo. Ver seção 9.4 | 13/08, superada 22/08 |
...
| Workflows de dados, ativos | **Sete**, conferidos em 08/08/2026: OLV2 Dados, OLV2 Pedido, OLV2 Estoque e OLV2 Clientes desde 30/07/2026; OLV2 Cadastros desde 31/07/2026; OLV2 Financeiro desde 01/08/2026, ampliado em 02, 03 e 07/08/2026; OLV2 Resultados desde 05/08/2026. **Cadastros, Estoque, Financeiro e Clientes republicados em 08/08/2026** para o módulo de Fornecedores, sem troca de identificador |
...
| Painel HTML atual | **Publicado.** Worker `painel-olv` no Cloudflare (`ffafb32017294e8ea1c09c86f2d01734`). Versão no ar em 22/08/2026: **`2026-08-22.1`**, lida no rodapé do menu lateral. Contém o Vale-Crédito completo, o ranking e o filtro de período da tela Clientes, a quitação de dívida no cartão em Contas a Receber, e a correção do resumo de pedido em Contas a Receber (seção 8.9) |
| Marcador de versão do painel | **Aplicado em 11/08/2026 (decisão 9), e em uso.** Constante `CONFIG.VERSAO` no topo do arquivo, no formato `AAAA-MM-DD` mais a sequência do dia, mostrada no rodapé do menu lateral ao lado do usuário logado. Encerrou a pendência 30. **Quem publicar tem de subir o número**, com a data real da publicação, não a data em que o código foi escrito. **Falhou uma vez, em 22/08/2026**: a publicação da manhã (quitação de dívida no cartão) foi ao ar sem subir o número, que ficou parado em `2026-08-14.2` mesmo com o arquivo mudado. Corrigido na publicação seguinte, no mesmo dia, para `2026-08-22.1` |
...
| POST /webhook/olv2-estoque | `{acao, token, tipo, produto_id, quantidade, custo_unitario, fornecedor, fornecedor_id}`. `fornecedor_id` desde 08/08/2026. Na Entrada, pode incluir `{gerar_conta_pagar, vencimento, parcela_total, periodicidade}` |
| POST /webhook/olv2-clientes | `{acao, token, nome, endereco, bairro, telefone}` ou `{acao, token, termo}` |
| POST /webhook/olv2-cadastros | `{acao, token, tabela, ...campos da tabela}`. Tabelas aceitas: `produtos`, `contas`, `maquininhas`, `bandeiras`, `taxas_cartao`, `formas_pagamento`, `categorias_conta_pagar` e **`fornecedores`** (08/08/2026) |
| POST /webhook/olv2-financeiro | Receber: `listar`, `criar`, `excluir`. Pagar: `listar_pagar`, `criar_pagar`, `editar_pagar`, `excluir_pagar`, os três últimos aceitando `fornecedor_id` desde 08/08/2026. Movimento: `baixar`, `estornar`, `fluxo_caixa`. Contas: `contas`, `ajustar_conta`, `conciliar_conta`, `estornar_ajuste_conta`. Filtros usam `{status, de, ate, termo, campo_data}`, com `campo_data` em `vencimento` (padrão), `emissao` ou `pagamento`; baixa usa um de `{conta_receber_id, conta_pagar_id}`, mais `{valor, data, forma, conta_id, observacao}` |
| POST /webhook/olv2-resultados | `{acao: resultados, token, de, ate, cliente, forma, produto}`. Exclusivo do administrador. O campo `cliente` também aceita o número do pedido |
...
| 22/08/2026 | 5.4 | **Quitação de dívida no cartão em Contas a Receber, Vale-Crédito passou a contar em Resultados na geração, e corrigido bug que quebrava o resumo de pedido em Contas a Receber.** (a) **Merge do avulso "Atualização 22-08-2026 Recebimento Cartão e Vale-Crédito"**, autorizado por Rogério ("já pode executar. já ficou claro o que será feito."). Baixa de Contas a Receber no Débito ou Crédito ganhou bandeira (Padrão/Outras) e parcelas, com a taxa da maquininha descontada do valor que entra na conta; a dívida continua quitada pelo valor cheio. Escopo fechado só em Contas a Receber, por decisão do Rogério; Contas a Pagar não muda. Correção vale só daqui para frente, sem recalcular o que já foi lançado. Seção 3.13. (b) **Tabela `baixas` ganhou as colunas de cartão** que `pagamentos` já tinha, com trava nova impedindo que apareçam numa baixa de Contas a Pagar. Funções `registrar_baixa` e a nova `registrar_baixas` (várias formas numa baixa só) capturam a taxa do mesmo jeito que a venda. `conta_movimentos` passou a creditar o valor líquido, não o cheio, no cartão. (c) **`vw_resultados` ganhou duas linhas de tipo**, Geração e Estorno de Vale-Crédito: a geração do crédito passou a contar como faturamento e lucro no dia em que o dinheiro entra (já com a taxa do cartão descontada, quando for o caso), e o consumo depois conta só o custo do produto, sem contar o valor de novo. **Fecha a pendência 32**, a de maior risco financeiro da lista, e supera a decisão 15 do quadro do Vale-Crédito, com autorização explícita do Rogério. Seção 3.9. (d) **Tela Resultados ganhou as duas opções novas no filtro de Tipo**, com selo de cor própria. Seção 5.11. (e) **Bug encontrado no mesmo dia, sem relação com o item (a): "Pedido não encontrado" em toda linha de Contas a Receber ao clicar no título.** O resumo de pedido (usado também em Vendas, Pedidos e Resultados) nunca soube procurar um pedido nos dados da própria tela Contas a Receber; bastava abrir o painel e ir direto para lá, sem visitar as outras três telas antes nesta sessão, para o pedido não ser achado em lugar nenhum. Conferido no n8n antes de mexer em qualquer código: todas as chamadas do dia ao financeiro tinham terminado com sucesso, então não era erro de banco nem de servidor. Corrigido no mesmo padrão já usado para a tela Resultados em 07/08/2026, com um cuidado a mais: o botão "Editar pedido" só aparece quando os itens do pedido realmente vieram junto, para não gravar um pedido de verdade por cima com um item em branco. Nova seção 8.9. (f) **Corrigido também**: a publicação da manhã tinha esquecido de subir o marcador `CONFIG.VERSAO` (regra 30); ficou em `2026-08-14.2` com o arquivo já mudado. Corrigido para `2026-08-22.1` na publicação que já sai com o item (e). (g) Arquivo avulso mesclado nesta versão; segue no projeto até Rogério confirmar a exclusão. |
| 22/08/2026 | 5.5 | **Registrado o backlog de melhorias de uso pedidas por Rogério, ainda sem nenhuma execução.** Nova seção 9.8: cinco pedidos enviados por Rogério depois de usar o painel no dia a dia (quatro prints e uma lista), com dificuldade e modelo de IA indicado para cada um. Quatro vão para o Sonnet 5 (mensagem e botão Excluir do resumo de pedido, ícones em Clientes/Contas a Pagar/Contas a Receber, filtro de forma de pagamento em Contas a Receber, Fornecedor com o mesmo comportamento de Cliente nos formulários de Estoque e de Contas a Pagar); um vai para o Opus 5 (transferência entre contas na tela Contas, por ser decisão financeira difícil de desfazer). Ordem combinada: Sonnet primeiro, Opus depois de o Sonnet publicar. Nenhuma tabela, função ou tela foi alterada nesta versão. |
