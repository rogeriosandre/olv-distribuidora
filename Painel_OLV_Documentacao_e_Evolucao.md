...
**Nota (28/07/2026)**: o papel antes chamado "vendedor" foi renomeado para "colaborador" em todo o sistema (painel, fluxo de contas e tabela de usuários). É uma mudança de rótulo apenas; nenhuma regra de permissão foi alterada. O nome de usuário de login "vendedor" (conta inicial) não mudou, só o papel associado a ele.
...
**Contas iniciais**: Rogério (administrador), Gabriele (administradora) e Vendedor (colaborador). Todas começam com senha temporária, trocada no primeiro acesso. A criação de contas é feita só pelo administrador (não há autocadastro na tela de login).
...
**Atualização (28/07/2026)**: o rótulo do papel "vendedor" foi renomeado para "colaborador" em todo o sistema. O histórico da decisão original (Opção A escolhida) é mantido abaixo sem alteração de conteúdo, só de terminologia.
...
- Login: opção A ou B; quais pessoas e (se A) quais papéis. **RESOLVIDO: Opção A, Rogério e Gabriele administradores, Vendedor colaborador.**
...
| 28/07/2026 | 1.7 | Papel "vendedor" renomeado para "colaborador" em todo o sistema: HTML do painel (OLV Painel HTML), fluxo OLV Contas (4 pontos de valor padrão) e tabela OLV Usuarios (papel do usuário "vendedor" atualizado). Mudança de rótulo apenas; nenhuma regra de permissão foi alterada. OLV Login e as travas de dono em OLV Vendas/OLV Estoque não precisaram de alteração, pois já usavam comparação positiva sobre "admin". Nome de usuário de login "vendedor" mantido sem alteração. |
